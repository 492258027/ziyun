...
func MakeHTTPHandler(endpoints Endpoints, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("POST").Path("/opstring/{Type}").Handler(httptransport.NewServer(
		endpoints.OpstringEndpoint,
		DecodeHTTPOpstringZeroRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
	return m
...
// DecodeHTTPOpstringZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded opstring request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPOpstringZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.OpstringRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
	...
	TypeOpstringStr := pathParams["Type"]
	TypeOpstring := TypeOpstringStr
	req.Type = TypeOpstring
