...
func New(instance string, options ...httptransport.ClientOption) (pb.AuthServer, error) {
	...
	var AuthZeroEndpoint endpoint.Endpoint
	{
		AuthZeroEndpoint = httptransport.NewClient(
			"POST",
			copyURL(u, "/auth/"),
			EncodeHTTPAuthZeroRequest,
			DecodeHTTPAuthResponse,
			options...,
		).Endpoint()
	}
...
// DecodeHTTPAuthResponse is a transport/http.DecodeResponseFunc that decodes
// a JSON-encoded AuthResponse response from the HTTP response body.
// If the response has a non-200 status code, we will interpret that as an
// error and attempt to decode the specific error message from the response
// body. Primarily useful in a client.
func DecodeHTTPAuthResponse(_ context.Context, r *http.Response) (interface{}, error) {
	defer r.Body.Close()
	buf, err := ioutil.ReadAll(r.Body)
	if err == io.EOF {
		return nil, errors.New("response http body empty")
...
// EncodeHTTPAuthZeroRequest is a transport/http.EncodeRequestFunc
// that encodes a auth request into the various portions of
// the http request (path, query, and body).
func EncodeHTTPAuthZeroRequest(_ context.Context, r *http.Request, request interface{}) error {
	strval := ""
	_ = strval
	...
	toRet := request.(*pb.AuthRequest)

	toRet.CompanyID = req.CompanyID

	toRet.UserID = req.UserID

	toRet.UserName = req.UserName

	toRet.Passwd = req.Passwd

	toRet.AToken = req.AToken

	toRet.RToken = req.RToken
